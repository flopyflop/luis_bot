...
class MainDialog(ComponentDialog):
    ...
    async def final_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        if step_context.result is not None:
            ...
            if isinstance(result, BookingDetails):
                msg_txt = f"User ID: {result.user_id} I have you booked to {result.destination} from {result.origin} on {result.travel_date} for " \
                      f"{result.capacity} passengers "
            if isinstance(result, CancelBookingDetails):
                msg_txt = f"User ID: {result.user_id} I have deleted a reservation to {result.destination}  on {result.travel_date} "

            message = MessageFactory.text(msg_txt, msg_txt, InputHints.ignoring_input)
            await step_context.context.send_activity(message)
        else:
            msg_txt = f"Could not find matching flight "
            message = MessageFactory.text(msg_txt, msg_txt, InputHints.ignoring_input)
            await step_context.context.send_activity(message)
